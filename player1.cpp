...
void Player::update(float dt)
{
    ...
    if (!otherIsMoving && getBoundingBox().intersectDown(otherBox)) {
        float pushOffset = otherBox.intersectSideways(futureBox);
        otherPlayer->position_x += pushOffset;
        printf("Attempting to jump on the other player\n");
    }
    position_y = newY;
...
bool Player::isShootingHigh() const {
    return shootingHigh;
}

bool Player::isShootingLow() const {
    return shootingLow;
}
